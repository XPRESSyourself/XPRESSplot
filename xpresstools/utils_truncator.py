...
def truncate(
    input_gtf,
    truncate_amount=45,
    save_coding_path='./', save_truncated_path='./',
    sep='\t', return_files=False):
...
"""GTF truncation function"""
def execute_truncator(
    gtf, truncate_amount):

    gtf['plus'] = gtf[[2,3,4,6,8]].apply(lambda x:
                    (x[3] + truncate_amount) \
                    if x[2] == "exon" \
                        and x[3] + truncate_amount <= x[4] \
                        and x[6] == "+" \
                        and "exon_number \"1;" \
                    in x[8] \
                    else ("delete_this" \
